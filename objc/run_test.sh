...
while true; do
    case $1 in
      --list) challenges=$(ls -1 . | grep -vwE "($OUT_DIR|run_test.sh)")
...
if [ -z "$challenges" ]; then
	challenges=$(ls -1 . | grep -vwE "($OUT_DIR|run_test.sh)")
fi
...
for challenge in $challenges; do
	...
	if [ -d "./$challenge" ]; then
		clang -framework Foundation $challenge/main.m -o $OUT_DIR/$challenge
		...
		for i in {0..99}; do
			...
			if [ -f "$input" ] ; then
				...
				OUTPUT_PATH=$cal_output gtimeout $TEST_TIMEOUT $OUT_DIR/$challenge < $input
				test_exitcode=$?
